...
class WeekEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		weekThing = new MenuItem(0, 0, weekFileName);
		weekThing.y += weekThing.height + 20;
		weekThing.antialiasing = ClientPrefs.globalAntialiasing;
		add(weekThing);

		weekIcon = new FlxSprite(90,140);
	...
	function addEditorBox() {
		var tabs = [
			{name: 'Custom Week', label: 'Custom Week'},
			{name: 'Others Settings', label: 'Others Settings'},
		];
		...
		UI_box.selected_tab_id = 'Custom Week';
		add(UI_box);
	...
	var overlayInputText:FlxUIInputText;
	var weekNameInputText:FlxUIInputText;
	var weekFileInputText:FlxUIInputText;
	var weekIconInputText:FlxUIInputText;
	var hideCheckbox:FlxUICheckBox;

	public static var weekFileName:String = 'VsGhost';
	
	function addWeekUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Custom Week";
		
		songsInputText = new FlxUIInputText(10, 30, 200, '', 8);
		blockPressWhileTypingOn.push(songsInputText);

		backgroundInputText = new FlxUIInputText(10, songsInputText.y + 40, 120, '', 8);
		blockPressWhileTypingOn.push(backgroundInputText);

		overlayInputText = new FlxUIInputText(10, backgroundInputText.y + 40, 120, '', 8);
		blockPressWhileTypingOn.push(backgroundInputText);

		weekNameInputText = new FlxUIInputText(10, overlayInputText.y + 40, 150, '', 8);
		...
		weekIconInputText = new FlxUIInputText(10, weekFileInputText.y + 40, 120, '', 8);
		blockPressWhileTypingOn.push(weekIconInputText);
		reloadWeekThing();

		hideCheckbox = new FlxUICheckBox(10, weekIconInputText.y + 40, null, null, "Hide Week from Story Mode?", 100);
		...
		tab_group.add(new FlxText(overlayInputText.x, overlayInputText.y - 18, 0, 'Background Overlay Asset:'));
		...
		tab_group.add(new FlxText(weekIconInputText.x, weekIconInputText.y - 18, 0, 'Week Icon Preview:'));

		tab_group.add(songsInputText);
		tab_group.add(backgroundInputText);
		tab_group.add(overlayInputText);
		tab_group.add(weekNameInputText);
		tab_group.add(weekFileInputText);
		tab_group.add(weekIconInputText);
	...
	function addOtherUI() {
		var tab_group = new FlxUI(null, UI_box);
		tab_group.name = "Others Settings";
	...
	function reloadAllShit() {
		...
		overlayInputText.text = weekFile.weekOverlay;
		weekNameInputText.text = weekFile.weekName;
		weekFileInputText.text = weekFileName;
		weekIconInputText.text = weekFile.weekIcon;
	...
	function reloadOVERLAY() {
		bgOverlaySprite.visible = true;
		var assetName:String = weekFile.weekOverlay;

		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('images/storymenu/' + assetName)) || #end
			Assets.exists(Paths.getPath('images/storymenu/' + assetName + '.png', IMAGE), IMAGE)) {
				bgOverlaySprite.loadGraphic(Paths.image('storymenu/' + assetName));
				isMissing = false;
			}
		}
	...
	function reloadIcon() {
		...
		if(assetName != null && assetName.length > 0) {
			if( #if MODS_ALLOWED FileSystem.exists(Paths.modsImages('storymenu/' + assetName)) || #end
			Assets.exists(Paths.getPath('images/storymenu/' + assetName + '.png', IMAGE), IMAGE)) {
				weekIcon.loadGraphic(Paths.image('storymenu/' + assetName));
				isMissing = false;
			}
		}
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			if(sender == weekFileInputText) {
				...
				reloadBG();
			} else if(sender == overlayInputText) {
				weekFile.weekOverlay = overlayInputText.text.trim();
				reloadOVERLAY();
			} else if(sender == weekIconInputText) {
				weekFile.weekIcon = weekIconInputText.text.trim();
				reloadIcon();
