...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Bitchless!!', 0.2], //From 0% to 19%
		['Failure!!', 0.4], //From 20% to 39%
		['Disgusting', 0.5], //From 40% to 49%
		['Terrible', 0.6], //From 50% to 59%
		['Bruh', 0.69], //From 60% to 68%
		['Meh', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Sick', 0.9], //From 80% to 89%
		['Badass!', 1], //From 90% to 99%
		['Cathartic!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 19, 400, "", 42);
		timeTxt.setFormat(Paths.font("inversionz.otf"), 42, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("blue.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

		botplayTxt = new FlxText(0, healthBarBG.y + 40, FlxG.width, "AUTOPLAY", 32);
		botplayTxt.setFormat(Paths.font("blue.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function startSong():Void
	{
		...
		if(botplayTxt.visible) {
			FlxTween.tween(scoreTxt, {alpha: 0}, 0.1, {ease: FlxEase.quadInOut});
		} else {
			FlxTween.tween(scoreTxt, {alpha: 1}, 0.2, {ease: FlxEase.quadInOut});
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if(botplayTxt.visible) {
			//FlxTween.tween(scoreTxt, {alpha: 0}, 1, {ease: FlxEase.quadInOut});
		...
		iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) + (150 * iconP1.scale.x - 150) / 2 - iconOffset + 5;
		iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (150 * iconP2.scale.x) / 2 - iconOffset * 2 - 5;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.y -= 60;
		rating.acceleration.y = 650 * playbackRate * playbackRate;
		rating.velocity.y -= FlxG.random.int(150, 185) * playbackRate;
		...
		comboSpr.acceleration.y = FlxG.random.int(300, 400) * playbackRate * playbackRate;
		comboSpr.velocity.y -= FlxG.random.int(150, 170) * playbackRate;
		...
		for (i in seperatedScore)
		{
			...
			numScore.acceleration.y = FlxG.random.int(300, 400) * playbackRate * playbackRate;
			numScore.velocity.y -= FlxG.random.int(150, 170) * playbackRate;
